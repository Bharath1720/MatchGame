...
class MatchGameItem extends Component {
  ...
  getFilteredGameItems = () => {
    const {activeTabId} = this.state
    const filtered = imagesList.filter(each => each.category === activeTabId)
    return filtered
  }
  ...
  imageClicked = idVal => {
    ...
    const {id} = imgUrl
    // console.log(id)
    const filteredImages = imagesList.filter(each => each.id === idVal)
    const firstObject = filteredImages[0].id
  ...
  render() {
    const {activeTabId, isTimerRunning, imgUrl, score, seconds} = this.state
    const {imageUrl} = imgUrl
    const filteredGameItems = this.getFilteredGameItems()

    // const randomImage = Math.floor(Math.random() * imagesList.length)
    // const initialImage = imagesList[randomImage]
    // const {imageUrl, thumbnailUrl} = initialImage
    ...
    return (
      <div className="app-container">
        ...
        {isTimerRunning ? (
          <MatchScoreCard score={score} playAgain={this.resetBtn} />
        ) : (
          <div className="match-game-body">
            <img className="initial-image" src={imageUrl} alt="match" />
            ...
            <ul className="games-list">
              {filteredGameItems.map(eachImage => (
                <MatchGame
